...
public class EscapeJsonFilterTest {
  ...
  public void testHandlesNonPrintableCharacters() {
    ...
    assertThat(jinjava.render("{{ string|escapejson }}", vars))
      .isEqualTo("M\\u00134 \\b");
  }

  @Test
  public void testHandlesWhitespace() {
    assertThat(jinjava.render("{{ 'Testing\nlinebreak\n'|escapejson }}", new HashMap<>()))
      .isEqualTo("Testing\\nlinebreak\\n");
    assertThat(jinjava.render("{{ 'Testing\ttabbing\t'|escapejson }}", new HashMap<>()))
  ...
  public void testHandleSingleQuote() {
    Map<String, String> vars = ImmutableMap.of(
      "string",
      "Testing a 'single quote' for the week"
    );
    assertThat(jinjava.render("{{ string|escapejson }}", vars))
      .isEqualTo("Testing a 'single quote' for the week");
  }
